...
from src.processing.buy_analyzer import get_buy_activity as _get_buy_activity
from src.processing.sell_analyzer import get_sell_activity as _get_sell_activity
...
def get_buy_history(manager_name, limit_quarters=8):
    """
    Get buy activity history for a manager.
    
    Args:
        manager_name: Fund manager name
        limit_quarters: Limit to N most recent quarters (default 8 = 2 years)
    
    Returns:
        List of activity records with quarter, stock, activity type, share changes, etc.
    """
    return _get_buy_activity(manager_name, limit_quarters=limit_quarters)


def get_sell_history(manager_name, limit_quarters=8):
    """
    Get sell activity history for a manager.
    ...
    Returns:
        List of activity records with quarter, stock, activity type, share changes, etc.
    """
    return _get_sell_activity(manager_name, limit_quarters=limit_quarters)


def get_quarterly_changes_formatted(manager_name):
    """
    Get formatted quarterly changes (NEW, INCREASED, DECREASED, EXITED).
    Returns data in the same format as buy/sell activity for consistent UI display.
    
    Args:
        manager_name: Fund manager name
    
    Returns:
        List of activity records with quarter, stock, activity type, value changes, etc.
    """
    session = get_session()
    ...
    for company_name, recent_holding in recent_map.items():
        if company_name not in previous_map:
            ...
            activity_list.append({
                ...
                'portfolio_weight_change': round(recent_weight, 2),
            })
    
    # Find INCREASED/DECREASED/EXITED positions
    for company_name, previous_holding in previous_map.items():
        if company_name in recent_map:
            recent_holding = recent_map[company_name]
            if recent_holding.value > previous_holding.value:
                # INCREASED
                pct_increase = ((recent_holding.value - previous_holding.value) / previous_holding.value) * 100
                ...
                activity_list.append({
                    ...
                    'company_name': company_name,
                    'ticker': recent_holding.ticker,
                    'activity': f'Add {pct_increase:.1f}%',
            ...
            elif recent_holding.value < previous_holding.value:
                # DECREASED
                pct_decrease = ((previous_holding.value - recent_holding.value) / previous_holding.value) * 100
                previous_weight = (previous_holding.value / previous_portfolio_value) * 100
                recent_weight = (recent_holding.value / recent_portfolio_value) * 100
                weight_change = recent_weight - previous_weight
                ...
                activity_list.append({
                    ...
                    'company_name': company_name,
                    'ticker': recent_holding.ticker,
                    'activity': f'Reduce {pct_decrease:.1f}%',
            ...
            activity_list.append({
                'quarter': quarter_str,
                'period_date': recent_filing.period_of_report.strftime('%Y-%m-%d'),
                'stock': previous_holding.company_name,
                ...
                'previous_shares': previous_holding.shares,
                'current_shares': 0,
                'share_value': previous_holding.value,
                'portfolio_weight_change': round(-previous_weight, 2),
            })
